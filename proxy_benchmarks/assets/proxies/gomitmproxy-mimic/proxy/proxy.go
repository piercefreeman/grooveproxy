...
func (p *Proxy) Serve(l net.Listener) {
	log.Printf("start listening to %s - mimic", l.Addr())
	err := p.serve(l)
	if err != nil {
...
func (p *Proxy) handleTunnel(session *Session) error {
	...
	if session.ctx.IsMITM() {
		// @pierce - override for UClient mimicing of the hello client signature
		log.Printf("id=%s: overriding client hello signature", session.ID())
		/*tlsConn := tls.UClient(
			conn,
			&tls.Config{
		...
		log.Printf("No MITTM connection: %s", session.ID())
	}
...
func (p *Proxy) handleConnect(session *Session) error {
	...
	if p.canMITM(session.req.URL.Host) {
		...
		if b[0] == 22 {
			tlsConn := tls.Server(
				pc,
				p.MITMConfig.NewTLSConfigForHost(session.req.URL.Host),
			)
